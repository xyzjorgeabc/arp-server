...
export function fetchAlbaranVenta(req: Request, res: Response, next: Function){

  const id = req.body.albaran_venta.id;
  const idSerie = req.body.albaran_venta.id_serie;

  if(id === 'all' && !isNaN(idSerie)){
    const q = dbconn.format('SELECT *, DATE_FORMAT(fecha, "%d-%m-%Y") as fecha FROM albaranes_venta WHERE id_serie = ?;', [idSerie]);
    dbconn.query(q,
    ...
    const alb_q = dbconn.format('SELECT *, DATE_FORMAT(fecha, "%d-%m-%Y") as fecha FROM albaranes_venta WHERE id_serie = ? ORDER BY id DESC LIMIT 1;', [idSerie]);
    let albaran: Albaran;
    let registros: any[];
    ...
    function(err, result, fields){
      if(err || result.length === 0) res.status(404), res.send();
        ...
        const reg_q = dbconn.format('SELECT * FROM registros_albaran_venta WHERE id_serie_albaran = ? and id_albaran = ?;', [idSerie, albaran.id]);
        dbconn.query(reg_q,
          function(err, result, fields){
            if(err || result.length === 0) res.status(404), res.send();
            else {
